import Popover, { ArrowContainer, ContentRenderer } from 'react-tiny-popover';
import React, { PureComponent } from 'react';
export class ShareButton extends PureComponent<{}, {isPopoverShown: boolean}> {
  state = {
  ...
  render() {
    return (
      <Popover 
        isOpen={this.state.isPopoverShown}
        content={this.renderPopoverContent}
        position="top"
        padding={10}
        onClickOutside={this.handleClickOutside}
      >
        <button className="button share" onClick={this.handleClick}>↩ Share</button>
      </Popover>
    );
  }

  renderPopoverContent: ContentRenderer = ({ position, targetRect, popoverRect }) => {
